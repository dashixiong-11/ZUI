...
const Dialog: React.FC<{ onClose: () => void }> = (props) => {
  ...
  return ReactDOM.createPortal((
    <div className="orz-citySelect-dialog">
      <header>
      ...
      <CurrentLocation/>
      <h2>全部城市</h2>
      <ol className="orz-citySelect-index">
        {indexList.map(a => <li key={a}>{a}</li>)}
      </ol>
      <div className="cityList">所有城市</div>
      {cityList.map(([letter, list]) => {
        return (
          <div key={letter} className="orz-citySelect-citySection">
            <h4 data-letter={letter}>{letter}</h4>
            {list.map(city =>
              <div className="orz-citySelect-cityName" key={city}
