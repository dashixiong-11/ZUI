...
const scopedClass = scopedClassNameMaker('tree');
    ...
    useUpdate(expanded, () => {
        ...
        if (expanded) {
            ...
            const afterExpand = () => {
                ...
                divRef.current.classList.add('orz-tree-children-present');
            ...
            const afterCollapse = () => {
                ...
                divRef.current.style.height = '';
                divRef.current.classList.add('orz-tree-children-gone');
    ...
    return <div key={item.value} className={sc(classes)}>
        ...
        <div ref={divRef} className={sc({children: true, collapsed: !expanded})}>
            {item.children && item.children.map(sub =>
                <TreeItem key={sub.value} item={sub} level={level + 1} onItemChange={onItemChange}
                          treeProps={treeProps}/>
            )}
        </div>
    </div>;
